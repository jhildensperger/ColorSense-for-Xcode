...
- (id)init
{
    if (self = [super init]) {
        ...
        _whiteUIColorSwiftRegex = [NSRegularExpression regularExpressionWithPattern:@"(UIColor\\(white:)\\s*([0-9]*\\.?[0-9]*f?)\\s*(\\/\\s*[0-9]*\\.?[0-9]*f?)?\\s*,\\s+alpha:\\s*([0-9]*\\.?[0-9]*f?)\\s*(\\/\\s*[0-9]*\\.?[0-9]*f?)?\\s*\\)" options:0 error:NULL];
        ...
        _hexUIColorRegex = [NSRegularExpression regularExpressionWithPattern:@"\\[\\[UIColor.*initWithHex:\\s*(0[xX][0-9a-fA-F]+)\\s*,?\\s*(?:alpha:)?\\s*(\\d+\\.?\\d*)?\\s*\\]" options:0 error:NULL];
        _hexUIColorSwiftRegex = [NSRegularExpression regularExpressionWithPattern:@"UIColor\\(hex:\\s*(0[xX][0-9a-fA-F]+)\\s*,?\\s*(?:alpha:)?\\s*(\\d+\\.?\\d*)?\\s*\\)" options:0 error:NULL];
    }
    return self;
}
...
- (NSColor *)colorInText:(NSString *)text selectedRange:(NSRange)selectedRange type:(OMColorType *)type matchedRange:(NSRangePointer)matchedRange
{
    ...
    if (!foundColor) {
        [_hexUIColorSwiftRegex enumerateMatchesInString:text options:0 range:NSMakeRange(0, text.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
            NSRange colorRange = [result range];
